...
* fix(tedapi): `available_blocks` always 0 (read from wrong payload); `PINV_GridState` always None (copy-paste); phantom `TESYNC--None--None`/`TESLA--None` vitals blocks on PW3; PW3 vitals None-crashes; POD/PINV bounds guards; lock timeouts return cached data instead of raising; 429/503 no longer misclassifies PW2 as PW3; sessions closed on reconnect and `close_session()`
