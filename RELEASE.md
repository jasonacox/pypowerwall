...
* feat(tedapi): Powerwall 3 temperatures are back. `temps()` (and the proxy's `/temps` and `/temps/pw`) now report the hottest battery-pack reading per Powerwall 3 and expansion pack; they returned `{}` on PW3 because the PW2 thermal-controller signal (`THC_AmbientTemp`) doesn't exist there. `vitals()` carries the full breakdown in degrees C: `HVP_PackTempMax`, `HVP_PackTempMin`, and `HVP_ShuntTemperature` on each `TEPOD--…` block, and the inverter's `PCH_AmbientTemp` (enclosure) and `PCH_heatsinkTemp` on each `TEPINV--…` block (`None` when unavailable; `PCH_heatsinkTemp` is passed through as delivered but reads a constant 45.45 on current firmware, so `temps()` doesn't use it). Hardware-validated on a PW3 leader + follower over WiFi TEDAPI and v1r LAN; `/temps/pw` `PWn` numbering matches `/pod`.
