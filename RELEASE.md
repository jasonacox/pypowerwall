...
* fix(tedapi): `get_pw3_vitals()` no longer fails outright when a component entry in the gateway payload is malformed (e.g. `None`) — the alert collection now guards it, and that battery still reports its energy.
