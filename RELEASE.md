...
* fix(tedapi): the v1r LAN recovery probe runs once per window. When the window opened, every concurrent getter (each holds its own per-method API lock) could pass the unguarded check and reconnect at the same time; `_connect_v1r()` also cleared the DIN for the whole reconnect (up to ~2x timeout), so other threads started yet another reconnect via `connect()` or signed LAN requests with DIN `None`, and the failure counter could be bumped once per racing thread, pushing the next probe out extra backoff steps. One thread now claims the probe under a small lock (never held across the network call) while the others keep routing via WiFi; the known DIN stays visible until the reconnect succeeds; and LAN failures of requests already in flight when the failover tripped no longer escalate it. Backoff schedule unchanged (`60 * 2**n` s, capped at 7680 s).
* fix(tedapi): a failed v1r LAN recovery probe no longer drops the DIN while a WiFi fallback is configured. When neither the LAN nor the WiFi host's `/tedapi/din` answered, the DIN was cleared, so every getter then ran a full reconnect on each poll (ignoring the probe backoff) and returned "Not Connected" instead of serving via WiFi once it answered again. Without a WiFi fallback the DIN is still cleared, keeping the per-poll reconnect that restores data soonest. The WiFi cold-failover state (#394) is now updated under the same lock. `connect()` still returns the DIN or `None`.
...
* tests: new `test_v1r_lan_probe.py`, `test_tedapi_hardening.py`, `test_v1r_transport_hardening.py`.
