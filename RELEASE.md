...
* feat(tedapi): firmware/system info now works on the `V2026_06` query set — `get_firmware_version()` is refactored onto a unified `_get_system_info()`/`SystemInfo` model that produces identical output across `V2024_06` and `V2026_06`, and across the basic and v1r transports. New TEDAPI CLI flags `-firmware`/`-details` expose it directly. (#356)
* fix(tedapi): v1r WiFi-fallback responses are now normalized at the transport boundary — when LAN has failed and `_post_tedapi()` transparently routes a primary query via the WiFi TEDAPI fallback, the full transport `Message` returned by WiFi is re-extracted to the bare `MessageEnvelope` bytes the v1r contract promises. Previously the wrapper was handed back as-is and v1r callers misparsed it (protobuf decodes leniently), yielding silently empty/incorrect data (e.g. blank firmware version) whenever the fallback fired. Normalizing inside `_post_tedapi()` fixes every caller at once and avoids racy `lan_failed` re-checks at parse time. (#356)
* **BREAKING (approved exception)** feat(tedapi): `get_firmware_version(details=True)` renames the opaque reverse-engineered keys `system.five` → `system.systemUpdate` and `system.six` → `system.deviceType`, and surfaces the previously-dropped `installedFirmwareSignature`/`offlineFirmwareSignature` fields. This is a deliberate, maintainer-approved exception to the never-change-return-shapes policy (see `AGENTS.md`): `five`/`six` were placeholder names from incomplete MITM reverse engineering — Tesla's own gateway frontend (`app.js`) documents these fields as `systemUpdate`/`deviceType`, so this restores their real names rather than carrying misleading aliases indefinitely. The fields were undocumented, carried no known consumers (not used by Powerwall-Dashboard, the pypowerwall facade, or the proxy), and protobuf field numbers are unchanged so wire decoding is identical. Anyone who did read `five`/`six` should switch to `systemUpdate`/`deviceType`. (#356)
