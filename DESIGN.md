...
|--------|----------------|
| `__init__.py` | `Powerwall` facade: mode selection, validation, all public convenience methods, `set_debug()`, `__version__`/`version_tuple` (derived from `pypowerwall/VERSION`) |
...
- Library version lives **only** in [pypowerwall/VERSION](pypowerwall/VERSION); both `setup.py` (build time) and `pypowerwall/__init__.py` (runtime) read it — `__version__`/`version_tuple` are derived, never hand-edited.
...
- Release order: bump `pypowerwall/VERSION` → update root `RELEASE.md` → `upload.sh` (PyPI) → bump `proxy/requirements.txt` pin + `BUILD` + `proxy/RELEASE.md` → `proxy/upload.sh` (Docker Hub; it verifies the pinned version exists on PyPI first).
...
7. **Version strings have exactly one home each** (library: `pypowerwall/VERSION`; proxy: `BUILD`).
