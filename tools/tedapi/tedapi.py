...
if __name__ == "__main__":
    ...
    # Create TEDAPI Object
    ted = TEDAPI(gw_pwd)
    # Get and save configuration to file
    config = ted.get_config()
    with open('config.json', 'w') as f:
        json.dump(config, f)
    # Get and save status to file
    status = ted.get_status()
    with open('status.json', 'w') as f:
        json.dump(status, f)
    print(" - Configuration and Status saved to config.json and status.json")
    ask = input(" - Do you want to print the configuration and status files? (y/N): ")
    if ask.lower() == 'y':
        print(" - Configuration:")
