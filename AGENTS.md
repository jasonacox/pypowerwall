...
- Release PR (maintainer): bump the `__version__` literal in `pypowerwall/__init__.py` (the single source of truth: `version`/`version_tuple` derive from it and `pyproject.toml` reads it at build time) and rename `## Unreleased` to `## vX.Y.Z - Title`, ending with a `* Library version bumped to \`X.Y.Z\`` bullet.
