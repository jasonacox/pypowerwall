...
- Library version: bump `__version__` in `pypowerwall/__init__.py` (the single source of truth; `version`/`version_tuple` derive from it, and `pyproject.toml` reads it at build time).
