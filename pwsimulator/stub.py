...
"""
 ...
 Powerwall Simulator Stub Tool
    ...
    TEDAPI (Powerwall 3):
    /tedapi/din - Return Device Identification Number
    /tedapi/v1 - Handle protobuf requests for config and status queries
...
import json
import os
import gzip  # For potential future compressed TEDAPI responses
try:
    import tedapi_pb2  # Protobuf definitions for TEDAPI
    TEDAPI_ENABLED = True
except ImportError:
...
# TEDAPI Sample Data
tedapi_din = "1232100-00-E--TG123456789ABC"
...
tedapi_config = {
    ...
    "battery_blocks": [
        {
            "vin": "2012170-25-E--TG123456789DEF",
            "type": "Powerwall2",
            "disabled_reasons": [],
        ...
        {
            "vin": "3012170-05-B--TG123456789GHI",
            "type": "Powerwall2",
            "disabled_reasons": [],
            "nominal_energy_remaining": 13500,
            "nominal_full_pack_energy": 13500
...
tedapi_status = {
    "control": {
        "systemStatus": {
            ...
            "nominalEnergyRemainingWh": 6500
        },
        "islanding": {
            "customerIslandMode": "offGrid",
            "contactorClosed": True,
            "microGridOK": True,
            "gridOK": False
        },
        "meterAggregates": [
            {"location": "SITE", "realPowerW": -2100},
            {"location": "BATTERY", "realPowerW": -3500},
            {"location": "LOAD", "realPowerW": 900},
            {"location": "SOLAR", "realPowerW": 6500}
        ],
        "alerts": {
            "active": []
        },
        ...
        "batteryBlocks": [
            {"din": "2012170-25-E--TG123456789DEF", "disableReasons": []},
            {"din": "3012170-05-B--TG123456789GHI", "disableReasons": []}
        ],
        "pvInverters": [
            {"din": "1538100-00-F--C123456789JKL", "disableReasons": []}
        ]
    },
    "system": {
        "time": "2024-01-24 12:00:00 +0000",
        "sitemanagerStatus": {
            "isRunning": True
        },
        "updateUrgencyCheck": {
        ...
        "readings": [
            {
                "serial": "V1234567890",
                "dataRead": [
                    {"voltageV": 240.5, "realPowerW": 6500, "reactivePowerVAR": -100, "currentA": 27.1}
                ],
                "timestamp": 1706097600
            }
        ],
    ...
    "esCan": {
        "bus": {
            "PVAC": [
                {
                    "packagePartNumber": "1538100-00-F",
                    "packageSerialNumber": "C123456789JKL",
                    "PVAC_Status": {
                        "isMIA": False,
                        "PVAC_Pout": 6500,
                        "PVAC_State": 8,
                    ...
                    "PVAC_Logging": {
                        "isMIA": False,
                        "PVAC_PVCurrent_A": 12.5,
                        "PVAC_PVCurrent_B": 0.0,
                        "PVAC_PVCurrent_C": 14.2,
                        "PVAC_PVCurrent_D": 13.8,
                        "PVAC_PVMeasuredVoltage_A": 385.2,
                        "PVAC_PVMeasuredVoltage_B": -1.0,
                        "PVAC_PVMeasuredVoltage_C": 395.7,
                        "PVAC_PVMeasuredVoltage_D": 390.1,
                        "PVAC_VL1Ground": 120.2,
                        "PVAC_VL2Ground": 120.3
                    },
                    "alerts": {"active": []}
                }
            ],
            ...
            "THC": [
                {
                    ...
                    "packageSerialNumber": "TG123456789DEF",
                    "alerts": {"active": []}
                },
                {
                    "packagePartNumber": "3012170-05-B",
                    "packageSerialNumber": "TG123456789GHI",
                    "alerts": {"active": []}
                }
            ],
            "SYNC": {
                "packagePartNumber": "1493315-01-F",
                "packageSerialNumber": "J123456789MNO",
                "METER_X_AcMeasurements": {
                    "METER_X_CTA_InstRealPower": 450,
                    "METER_X_CTB_InstRealPower": 450,
                    "METER_X_VL1N": 120.2,
                    "METER_X_VL2N": 120.3
                },
                "alerts": {"active": []}
            ...
            "ISLANDER": {
                ...
                "ISLAND_AcMeasurements": {
                    "ISLAND_VL1N_Main": 120.2,
                    "ISLAND_FreqL1_Main": 60.0,
                    "ISLAND_VL2N_Main": 120.3,
                    "ISLAND_FreqL2_Main": 60.0,
                    "ISLAND_GridState": 2
                }
            },
            "MSA": {
                "packagePartNumber": "1538000-00-E",
                "packageSerialNumber": "M123456789PQR",
                "METER_Z_AcMeasurements": {
                    "METER_Z_CTA_InstRealPower": -1050,
                    "METER_Z_CTB_InstRealPower": -1050,
                    "METER_Z_VL1G": 120.2,
                    "METER_Z_VL2G": 120.3
                },
                "alerts": {"active": []}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == '/tedapi/v1':
            ...
            try:
                # TEDAPI doesn't require session cookie - uses Basic Auth
                content_length = int(self.headers.get('Content-Length', 0))
                post_data = self.rfile.read(content_length)
                
                # Parse the incoming protobuf request
                request = tedapi_pb2.Message()
                request.ParseFromString(post_data)
                
                # Determine what's being requested based on the request structure
                response = tedapi_pb2.Message()
                
                if request.message.HasField('config') and request.message.config.HasField('send'):
                    # Config request
                    response.message.deliveryChannel = 1
                    response.message.recipient.local = 1
                    response.message.sender.din = tedapi_din
                    response.message.config.recv.file.name = request.message.config.send.file
                    response.message.config.recv.file.text = json.dumps(tedapi_config)
                    response.tail.value = 1
                    log_msg = "config"
                    
                elif request.message.HasField('payload') and request.message.payload.HasField('send'):
                    # Status/Controller query request
                    response.message.deliveryChannel = 1
                    response.message.recipient.local = 1
                    response.message.sender.din = tedapi_din
                    response.message.payload.recv.text = json.dumps(tedapi_status)
                    response.tail.value = 1
                    log_msg = "status/payload"
                    
                else:
                    # Unknown request type - return minimal response
                    response.tail.value = 1
                    log_msg = "unknown"
                
                print(f"TEDAPI v1 request ({len(post_data)} bytes) - returning {log_msg}", flush=True)
                ...
                import traceback
                print(f"Error handling TEDAPI request: {e}", flush=True)
    ...
    def do_GET(self):
        ...
        #
        # TEDAPI - DIN
        if self.path == '/tedapi/din':
            # TEDAPI doesn't require session cookie
            self.send_response(200)
            self.send_header('Content-type', 'text/plain')
            self.end_headers()
            message = tedapi_din
        ...
        if self.path == '/api/status':
            session_valid = False
            # Accept either session cookie OR Basic Auth (for TEDAPI mode)
            has_cookie = 'cookie' in self.headers and '1234567890qwertyuiopasdfghjklZXcvbnm' in self.headers['cookie']
            has_basic_auth = 'authorization' in self.headers or 'Authorization' in self.headers
            if has_cookie or has_basic_auth:
                # Valid Login
                self.send_response(200)
                self.send_header('Content-type', 'application/json')
                self.end_headers()
