...
class RoutePerformanceTester:
    ...
    def print_summary(self, results: List[Dict], show_errors: bool = False):
        ...
        if failed_results:
            print(f"\n  {len(failed_results)} routes FAILED:")
        ...
        if successful_results:
            ...
            for result in successful_results:
                ...
                impact = stats["avg_ms"] * usage / 1000
        ...
        if successful_results:
            ...
            for i, result in enumerate(caching_candidates[:10], 1):
                ...
                print(f"  {i:2d}. {result['route']:<35} ({stats['avg_ms']:.1f}ms x {result['usage_count']} = {impact:.0f}s impact)")
...
def main():
    ...
    parser.add_argument("--sort", choices=["avg", "usage", "impact"], default="impact",
                       help="Sort results by: avg (response time), usage (request count), impact (time x usage)")
