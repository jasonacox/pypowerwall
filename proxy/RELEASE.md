...
* New `PW_TEDAPI_AUTH_MODE=basic|bearer` environment variable (default `basic`): `basic` uses HTTP Basic Auth and needs a route to `192.168.91.1`; `bearer` logs in via `/api/login/Basic` and wraps queries in an AuthEnvelope, which works from the home network without a static route and is what Powerwall 3 requires. Pair `bearer` with `PW_TEDAPI_API_VERSION=V2026_06` — the bearer transport expects the Tesla-signed GraphQL query set.
