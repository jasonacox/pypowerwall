...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == '/aggregates' or self.path == '/api/meters/aggregates':
            # Meters - JSON
            message = cw.poll('/api/meters/aggregates')
        elif self.path == '/soe':
            # Battery Level - JSON
            message = cw.poll('/api/system_status/soe')
        elif self.path == '/api/system_status/soe':
            # Force 95% Scale
            level = cw.level(scale=True)
            message = json.dumps({"percentage":level})
        elif self.path == '/api/system_status/grid_status':
            # Grid Status - JSON
            message = cw.poll('/api/system_status/grid_status')
        elif self.path == '/csv':
            ...
            # Vitals Data - JSON
            message = cw.vitals(jsonformat=True)
        ...
        elif self.path in ['/api/status','/cloud']:
            #################### TEST ####################
            message = cw.poll(self.path)
            log.debug(" ** Cloud: {}".format(message))
            message2 = pw.poll(self.path)
            log.debug(" ** Local: {}".format(message2))
        elif self.path in [
            '/api/site_info/site_name', '/api/meters/site',
            '/api/meters/solar', '/api/sitemaster', '/api/powerwalls', 
            '/api/customer/registration', '/api/system_status', '/api/system_status/grid_status',
            '/api/system/update/status', '/api/site_info', '/api/system_status/grid_faults',
            '/api/operation', '/api/site_info/grid_codes', '/api/solars', '/api/solars/brands',
